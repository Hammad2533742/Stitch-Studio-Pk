...
export default function NeedleCursor() {
  ...
  useEffect(() => {
    ...
    const move = (e) => {
      ...
      y.set(e.clientY);
      const el = e.target;
      setActive(!!el?.closest?.('a, button, input, textarea, select, label, [role="button"], [data-cursor="active"]'));
    };
  ...
  return (
    <motion.div className="pointer-events-none fixed top-0 left-0 z-[100]" style={{ x: sx, y: sy }} aria-hidden="true">
      <motion.div
      ...
      >
        <svg
          width="18" height="40" viewBox="0 0 18 40" fill="none"
          className={active ? 'text-gold' : 'text-ink'}
          style={{ filter: 'drop-shadow(0 0 4px rgba(156,122,70,0.35))' }}
          aria-hidden="true"
        >
          ...
          <path d="M9 6 C 14 10, 13 18, 9 22" stroke="#9C7A46" strokeWidth="1" fill="none" strokeLinecap="round" />
        </svg>
      </motion.div>
    </motion.div>
  );
